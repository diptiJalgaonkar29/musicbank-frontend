...
const TrackResultView = (props) => {
  ...
  const getTrackMeta = async (trackFilters, spotifyId, trackTypes = []) => {
    ...
    try {
      ...
      //   formData
      // );
      setTracks(props?.trackData?.trackData);
    } catch (error) {
  ...
  }, [filterItemsMemo, spotifyId, trackTypeFilters?.length, tracks]);
  ...
  return (
    <>
      {isLoading ? (
        <div className="trackresultView_loading_container">
          <SpinnerDefault />
        </div>
      ) : (
        <main className="trackResultView_container">
          <div className="trackresultView_content_main">
            {/* <div
            ...
            <section className="browseV2_tracklist">
              {tracks?.length > 0 ? (
                ...
                <h1 className="browseV2_no_data_header">{
                  props?.status === "bad_request" ?
                    <FormattedMessage id={"AISearch.BadRequest"} /> : "No Tracks Found"}
                </h1>
              )}
            </section>
          </div>
...
export default TrackResultView;
