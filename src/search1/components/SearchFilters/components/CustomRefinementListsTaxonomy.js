...
const CustomRefinementListsTaxonomy = connectRefinementList(
  ({ items, refine, attribute }) => {
    ...
    //map labels and sort tags from a-z
    const itemsSorted = items
      .map((item) => ({
        ...item,
        label: getLabelByAttribute(attribute, item?.label),
      }))
      .sort((a, b) => {
        if (a.label < b.label) {
          return -1;
...
export default CustomRefinementListsTaxonomy;
