...
import "./ActiveProjectListItem.css";
...
export default function ProjectListItem({
  ...
  const handleRedirect = (id) => {
    // navigate(`/project-download/${id}`);
    navigate(`/track-download/${id}`, {
      state: { credits: creditValue || 0 },
    });
  };

  return (
    <div className="Activeproject_list_item">
      <div
        className="ActiveprojectName_container"
        onClick={() =>
      ...
      <div className="projectattr" style={{ width: "25%" }}>
        <p>{projectItem?.userName}</p>
      </div>
      {/* {["Active Projects", "Closed Projects"]?.includes(projectType) && (
        <div
